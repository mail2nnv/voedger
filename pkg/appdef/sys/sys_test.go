...
func Test_SysPackage(t *testing.T) {
	...
	t.Run("Should be ok to obtain empty app", func(t *testing.T) {
		...
		t.Run("should be ok to read sys views", func(t *testing.T) {
			v := appdef.View(app.Type, sys.Prp_ViewName)
			require.NotNil(v)
			require.NotNil(v.Key().PartKey().Field(sys.Prp_PID))
			require.NotNil(v.Key().ClustCols().Field(sys.Prp_WSID))
			require.NotNil(v.Value().Field(sys.Prp_Offset))
			require.NotNil(v.Value().Field(sys.Prp_BaseRecordID))
			require.NotNil(v.Value().Field(sys.Prp_CBaseRecordID))
		})
	})
}
