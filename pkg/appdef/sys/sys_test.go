...
func Test_SysPackage(t *testing.T) {
	...
	t.Run("Should be ok to obtain empty app", func(t *testing.T) {
		...
		t.Run("should be ok to read sys views", func(t *testing.T) {
			v := appdef.View(app.Type, sys.SequencesView)
			require.NotNil(v)
			require.NotNil(v.Key().PartKey().Field(sys.SequencesView_PID))
			require.NotNil(v.Key().ClustCols().Field(sys.SequencesView_WSID))
			require.NotNil(v.Key().ClustCols().Field(sys.SequencesView_Name))
			require.NotNil(v.Value().Field(sys.SequencesView_Last))
		})
	})
}
