...
func dynoStoreRow(row *Row, buf *bytes.Buffer) {
	id, err := row.QNameID()
	if err != nil {
		// no test
		panic(errs.Enrich(err, row))
	}
...
func dynoStoreRowSysFields(row *Row, buf *bytes.Buffer) {
...
func dynoLoadRow(row *Row, codecVer byte, buf *bytes.Buffer) (err error) {
	row.Clear()
...
func loadRowSysFields(row *Row, codecVer byte, buf *bytes.Buffer) (err error) {
